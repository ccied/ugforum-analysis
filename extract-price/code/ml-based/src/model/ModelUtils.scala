...
object ModelUtils {
  ...
  def evaluateTokenLevel(data: Seq[LabeledDocument], predictions: Seq[LabeledDocument], writeOutputForSig: Boolean): EvalStats = {
    ...
    for (i <- 0 until data.size) {
      ...
      // Logger.logss("Gold label size " + goldLabels + " Pred label size" + predLabels)
      
       var goldProducts: List[String] = List()
       var predProducts: List[String] = List()
       
       for (label <- predictions(i).positiveLabels.asScala) {
        
         predProducts = predProducts ++ getTaggedProduct(predictions(i).document, label.getFirst().intValue, label.getSecond().intValue).words
         Logger.logss(predictions(i).document.documentId + ",\""+getTaggedProduct(predictions(i).document, label.getFirst().intValue, label.getSecond().intValue).words(0) + "\"")
        
       }
        for (glabel <- data(i).positiveLabels.asScala) {
        //val predProducts = new HashSet[TaggedProduct]
      
        goldProducts = goldProducts ++ getTaggedProduct(data(i).document, glabel.getFirst().intValue, glabel.getSecond().intValue).words
        
//        Logger.logss("********Gold label******"+data(i).document.documentId + 
//            ",\""+getTaggedProduct(data(i).document, glabel.getFirst().intValue, glabel.getSecond().intValue).words + "\"" )
//        
      }
      val gset = goldProducts.toSet
      val pset = predProducts.toSet
      //Logger.logss("Gold label size " + gset + " Pred label size" + pset)
      
      
      
      
      val thisTp = (gset.intersect(pset)).size
      val thisFp = pset.size - thisTp
      val thisFn = gset.size - thisTp
      thisCorr += thisTp
      thisPred += pset.size
      thisGold += gset.size
...
//  def evaluateTokenLevelPrice(data: Seq[LabeledDocument], predictions: Seq[LabeledDocument], writeOutputForSig: Boolean): EvalStats = {
//    var tp = 0.0F
//    var fp = 0.0F
//    var fn = 0.0F
//    
...
////        //val predProducts = new HashSet[TaggedProduct]
////      
////        //predProducts+ = getTaggedProduct(data(i).document, label.getFirst().intValue, label.getSecond().intValue)
////        Logger.logss("********Gold label******"+data(i).document.documentId + 
////            ",\""+getTaggedProduct(data(i).document, glabel.getFirst().intValue, glabel.getSecond().intValue).words + "\"" )
////        
////      }
//        
//      //Logger.logss("product->"+predProducts)
