...
object MetricComputer {
  ...
  def fmtTwoDigitNumber(value: Double, numDecimalPlaces: Int) = {
//    val int = (value * Math.pow(10, 2 + numDecimalPlaces)).toInt
    val rawStr = value + ""
    if (!rawStr.contains(".")) {
      rawStr
    } else {
      rawStr.substring(0, Math.min(rawStr.size, rawStr.indexOf(".") + 1 + numDecimalPlaces))
    }
  }
    
  def fmtMetricValues(suffStats: Seq[Double]): String = {
    val strList = suffStats.map(entry => fmtTwoDigitNumber(entry, 2));
